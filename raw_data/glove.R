glove_raw <- data.table::fread("glove.840B.300d.txt", header = FALSE, quote = "")

bing_pos <- scan("raw_data/positive-words.txt", comment.char = ";", what = "character", quiet = TRUE)

bing_neg <- scan("raw_data/negative-words.txt", comment.char = ";", what = "character", quiet = TRUE)
...
usethis::use_data(glove_sweeney)
